(function (jQuery) {
	...
	const ShowJQueryAlert = (title, message, buttons, callback) => {
		$('body').append(`
	<dialog tabindex="-1" id="dialog" style="padding: 0px; border-radius: 8px; border-style: none; box-shadow: 2px 2px 2px rgba(0, 0, 0, 0.25); width: 320px">
		<h4 style="margin: 24px 24px 0px 24px;">${title}</h4>
		<p style="margin: 16px 24px;">${message}</p>
		<div id="actionView" style="margin: 0px 8px 8px; display: flex; flex-direction: row-reverse; flex-wrap: wrap;"></div>
	</dialog>
`);
